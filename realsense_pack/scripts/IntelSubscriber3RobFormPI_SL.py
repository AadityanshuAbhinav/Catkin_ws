...
global s,v, id_list, Coordinates_list,z_anglist,ite,initial_time,move1,move2,move3,poseid,poseidin,goallist,flaglist,Form,FormErrMat1,DistErrMat1,FormErrMat2,DistErrMat2,FormErrMat3,DistErrMat3,PI_error1,PI_error2,PI_error3,Traject,XPrev,YPrev,ZPrev
s = 0.5
v = np.array([0.03,0])
id_list = None
...
goallist = [[0.6,0.6],[0,0.6],[0.6,0.6]]
theta = np.pi/4
d = 0.4
Form = [[2*d*np.cos(theta),0],[-d*np.cos(theta),3*d*np.sin(theta)],[-d*np.cos(theta),-3*d*np.sin(theta)]]
...
class IntelSubscriber:
    ...
    def IntelSubscriberRGB(self,msg):
       global ite,flaglist,initial_time,move1,move2,move3,ctime_mat1,c_mat1,dist_mat1,ctime_mat2,c_mat2,dist_mat2,ctime_mat3,c_mat3,dist_mat3,RelPosition,poseidin, goallist,Form,FormErrMat1,DistErrMat1,FormErrMat2,DistErrMat2,FormErrMat3,DistErrMat3,PI_error1,PI_error2,PI_error3,v,Xprev,Yprev,Zprev
       if ite == 0:
       ...
       if ids is not None:
          for i in range(len(ids)):
             ...
             if depth_value > 0:
                ...
                Xprev[ids[i,0]-1,:-1] = Xprev[ids[i,0]-1,1:]
                Xprev[ids[i,0]-1,-1] = marker_x
                
                Yprev[ids[i,0]-1,:-1] = Yprev[ids[i,0]-1,1:]
                Yprev[ids[i,0]-1,-1] = marker_y
                
                Zprev[ids[i,0]-1,:-1] = Zprev[ids[i,0]-1,1:]
                Zprev[ids[i,0]-1,-1] = z_ang    
                # Robot ids
                ...
                if flaglist[ids[i,0]-1] == 0:
                   ...
                   poseid[ids[i,0]-1,0],poseid[ids[i,0]-1,1],poseid[ids[i,0]-1,2] = self.Filter(Xprev[ids[i,0]-1,:],Yprev[ids[i,0]-1,:],Zprev[ids[i,0]-1,:],ite)
       ...
       goal = self.Traj(current_time-initial_time,poseidin[4,0:2],v,2)
       Traject.append(goal)
       print("start",poseidin[4,0:2],"dt",current_time-initial_time,"goal", goal)
       
       self.RelController(poseid[4,:],RelPosition,Form[0][:],goal,FormErrMat1,DistErrMat1,move1,1,c_mat1,ctime_mat1,4,PI_error1)
       self.RelController(poseid[2,:],RelPosition,Form[1][:],goal,FormErrMat2,DistErrMat2,move2,0,c_mat2,ctime_mat2,2,PI_error2)
       self.RelController(poseid[3,:],RelPosition,Form[2][:],goal,FormErrMat3,DistErrMat3,move3,0,c_mat3,ctime_mat3,3,PI_error3)
       ...
       if (current_time > initial_time + 8 ):
          ...
          Trajectory = np.array(Traject)
          scipy.io.savemat('Aruco.mat', dict(idmat=id_mat1, Coodmat=Coordinates_mat1, zmat=z_ang_mat1, cmat1=c_mat11, ctimemat1 = ctime_mat11, timemat = time_mat1,distmat1 = dist_mat11, cmat2=c_mat22, ctimemat2 = ctime_mat22, distmat2 = dist_mat22,cmat3=c_mat33, ctimemat3 = ctime_mat33, distmat3 = dist_mat33,Disterr1 = distErr1,Formerr1 = FormErr1,Disterr2 = distErr2,Formerr2 = FormErr2,Disterr3 = distErr3,Formerr3 = FormErr3,Trajecto = Trajectory))
    ...
    def RelController(self,poseid,RelPose,Form,goallist,Formerror,Disterror,move,lead,c_mat,ctime_mat,idd,PI_error):
       dx = Form[0]
       dy = Form[1]
       l = 0.1
       goal_x = goallist[0]
       ...
       x_form_error = dx - RelPose[idd*5+2,0] - RelPose[idd*5+3,0] - RelPose[idd*5+4,0]
       y_form_error = dy - RelPose[idd*5+2,1] - RelPose[idd*5+3,1] - RelPose[idd*5+4,1]
       ...
       print("x_error",x_form_error,"y_error",y_form_error)
    ...
    def Traj(self,t,start,v,tr):
       if tr==1:
          ...
          pose = np.array([0.6*np.cos(0.01*t),0.6*np.sin(0.01*t)])
          return list(origin+pose)
          
    def Filter(self,Xval,Yval,Zval,ite):
       if ite>6:
