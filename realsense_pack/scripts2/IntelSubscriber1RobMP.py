...
class Server:
    def __init__(self,ID):
       ...
       self.map_mat = np.empty([300,400,num_of_iters])
    ...
    def lidar_callback(self, msg):
       ...
       if self.iter < num_of_iters-1:
           ...
           self.map_mat[:,:,self.iter]=self.map
           self.iter = self.iter + 1
       else:
           print("stop")
           
           if self.ID==2:
               scipy.io.savemat('Resp_bot2.mat', dict(ID = self.ID, t2=self.t_mat,tt2 = self.tt_mat, x2=self.x_mat, th2=self.theta_mat, O_t2=self.odom_t_mat,O_tt2=self.odom_tt_mat, O_v2=self.odom_v_mat, O_w2=self.odom_w_mat, O_x2=self.odom_x_mat,O_y2=self.odom_y_mat,O_zang2=self.odom_zang_mat, map_mat = self.map_mat))
           elif self.ID==3:
    ...
    def map_update(self):
        posex = self.odom_x
        posey = self.odom_y+0.5
        posez = self.odom_zang
        
        ang = np.arange(0,360)*np.pi/180
        mapx = posex + self.xx_mat[self.iter,:]*np.cos(ang+posez)
        mapy = posey + self.xx_mat[self.iter,:]*np.sin(ang+posez)
   
        MapX = np.ceil((mapy)*(299/1.8)).astype(int)
        MapY = np.ceil((mapx+0.6)*(399/2.4)).astype(int)
...
if __name__ == '__main__':
    try:
       ...
       #Map_sub = rospy.Subscriber('tb3_0/map', OccupancyGrid, server1.map_callback)
