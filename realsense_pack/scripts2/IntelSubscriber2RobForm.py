...
class Server:
    ...
    def lidar_callback(self, msg):
       ...
       if self.iter < num_of_iters-1:
           ...
           if self.ID==2:
               scipy.io.savemat('Resp_bot2.mat', dict(ID = self.ID, t1=self.t_mat,tt1 = self.tt_mat, x1=self.x_mat, th1=self.theta_mat, O_t1=self.odom_t_mat,O_tt1=self.odom_tt_mat, O_v1=self.odom_v_mat, O_w1=self.odom_w_mat, O_x1=self.odom_x_mat,O_y1=self.odom_y_mat,O_zang1=self.odom_zang_mat))
           elif self.ID==3:
               scipy.io.savemat('Resp_bot3.mat', dict(ID = self.ID, t1=self.t_mat,tt1 = self.tt_mat, x1=self.x_mat, th1=self.theta_mat, O_t1=self.odom_t_mat,O_tt1=self.odom_tt_mat, O_v1=self.odom_v_mat, O_w1=self.odom_w_mat, O_x1=self.odom_x_mat,O_y1=self.odom_y_mat,O_zang1=self.odom_zang_mat))
           elif self.ID==4:
               scipy.io.savemat('Resp_bot4.mat', dict(ID = self.ID, t1=self.t_mat,tt1 = self.tt_mat, x1=self.x_mat, th1=self.theta_mat, O_t1=self.odom_t_mat,O_tt1=self.odom_tt_mat, O_v1=self.odom_v_mat, O_w1=self.odom_w_mat, O_x1=self.odom_x_mat,O_y1=self.odom_y_mat,O_zang1=self.odom_zang_mat))
...
class IntelSubscriber:
    ...
    def IntelSubscriberRGB(self,msg):
       ...
       goal = self.Traj(time.time()-initial_time,poseidin[4,0:1],v,1)
       
       goalmat.append(goal)
       
       #self.controller(poseid[4,0],poseid[4,1],poseid[4,2],poseidin[4,0],poseidin[4,1],poseidin[4,2],ctime_mat,c_mat,dist_mat,xdot_mat,move1)
       self.RelController(poseid[4,:],RelPosition,Form[0][:],goal,FormErrMat1,DistErrMat1,move1,1,c_mat1,ctime_mat1,4,PI_error1,xdot_mat1,xerr_mat1)
       self.RelController(poseid[2,:],RelPosition,Form[1][:],goal,FormErrMat2,DistErrMat2,move2,0,c_mat2,ctime_mat2,2,PI_error2,xdot_mat2,xerr_mat2)       
    ...
    def RelController(self,poseid,RelPose,Form,goallist,Formerror,Disterror,move,lead,c_mat,ctime_mat,PI_error,x_dot,x_err):
       dx = Form[0]
       dy = Form[1]
       l = 0.13
       goal_x = goallist[0][0]
    ...
    def Traj(self,t,start,v,tr):
       if tr==1:
          ...
          pose = np.array([0.5*np.cos(w*t),0.5*np.sin(w*t)])
