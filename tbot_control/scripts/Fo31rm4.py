...
def controller(min_value_1,min_index_1,min_value_2,min_index_2): 
  global move
  global robot_x, robot_y, robot_theta, x_form_prev_err4, y_form_prev_err4,ite2,trer_data
  ...
  if(ite2 == 0):
  	x_form_prev_err4 = x_form_error
  	y_form_prev_err4 = y_form_error
  
  x_dot = K_for_p * (x_form_error) + K_tr*distance_to_goal_x + K_for_d * (x_form_error-x_form_prev_err4)/0.2
  y_dot = K_for_p * (y_form_error) + K_tr*distance_to_goal_y + K_for_d * (y_form_error-y_form_prev_err4)/0.2
  ...
  print("Tracking Error4", distance_to_pose)
  print("Formation Errorx4", x_form_error)
  print("Formation Errory4", y_form_error)
  print("angular dev4", np.sin(robot_theta+min_index_1))
  print("angular vel4", -K_ang*np.sin(robot_theta+min_index_1))
  print("robot angle4", robot_theta)
  print("lidar angle4", min_index_1)
  print("Robot state4 ", robot_x, " ", robot_y)
...
def lidar_callback(msg):
  global tlidar_data, lidar_data, d, P_prev1, P_prev2, M_prev1, M_prev2, ite2, Z11, Z12, Z21, Z22, M1, Z1prev1, Z1prev2, Z1prev3, M2, Z2prev1, Z2prev2, Z2prev3,trer_data
  ...
  if (t_l > tlidar_data[0]+8):
    ...
    scipy.io.savemat('lid_robot4_FORM3.mat', dict(t1lid4=tlidar_mat, lid4=lidar_mat))
  ...
  if (t_l > tlidar_data[0]+8):
    ...
    scipy.io.savemat('Kallid_robot4_FORM3.mat', dict(lid4M11=lidarM1_mat, lid4M21=lidarM2_mat, lid4Z11=lidarZ1_mat, lid4Z21=lidarZ2_mat))
  
  print(" Measurement41 " , Z1," Measurement42 " , Z2)
  print(" Prediction41 " , M1," Prediction42 " , M2)
  ...
  if (t_l > tlidar_data[0]+8):
  	trer4_mat = np.array(trer_data)
  	
  	scipy.io.savemat('TRER_robot4_FORM3.mat', dict(trer4=trer4_mat))
...
def odom_callback(data):
  ...
  if (t_o > todom_data[0]+8):
    ...
    scipy.io.savemat('odom_robot4_FORM3.mat', dict(t41=todom_mat, x41=x_mat, y41=y_mat, th41=theta_mat))
  #print("Robot state",robot_theta)
  
  
##########################################################

rospy.init_node('Form34')  # Defines a node with name of Follower
velocity_pub = rospy.Publisher('tb3_2/cmd_vel', Twist, queue_size=10)
pose_subscriber = rospy.Subscriber('tb3_2/odom', Odometry, odom_callback)
lidar_subscriber = rospy.Subscriber('tb3_2/scan', LaserScan, lidar_callback)
rate = rospy.Rate(10)
