...
def controller(min_value_1,min_index_1,min_value_2,min_index_2): 
  ...
  d = 0.5
  dx  = -d*np.cos(30*np.pi/180)
  dy  = -3*d*np.sin(30*np.pi/180)
  l = 0.05
  ...
  x_dot = K_for * (x_form_error) + K_tr*distance_to_goal_x
  y_dot = K_for * (y_form_error)+ K_tr*distance_to_goal_y
  
  distance_to_pose = np.sqrt(x_form_error**2+y_form_error**2)
  
  if distance_to_pose<0.01:
    move.linear.x = 0
    move.angular.z = K_ang*np.sin((robot_theta+min_index_1)*np.pi/180)
  else:
  ...
  print("angular dev", np.sin(robot_theta+min_index_1))
  print("angular vel", -K_ang*np.sin(robot_theta+min_index_1))
  print("robot angle", robot_theta)
  print("lidar angle", min_index_1)
  print("Robot state ", robot_x, " ", robot_y)
