...
def lidar_callback(msg):
  ...
  if (t_l > tlidar_data[0]+8):
    ...
    scipy.io.savemat('lid_robot4_1.mat', dict(t1lid=tlidar_mat, lid=lidar_mat))
  ...
  goal_x = robot_x + ((min_value_1-d)*numpy.cos((min_index_1+robot_theta)*numpy.pi/180) + (min_value_2-d)*numpy.cos((min_index_2+robot_theta)*numpy.pi/180))/2
  goal_y = robot_y + ((min_value_1-d)*numpy.sin((min_index_1+robot_theta)*numpy.pi/180) + (min_value_2-d)*numpy.sin((min_index_2+robot_theta)*numpy.pi/180))/2
...
def odom_callback(data):
  ...
  if (t_o > todom_data[0]+8):
    ...
    scipy.io.savemat('odom_robot4.mat', dict(t1=todom_mat, x1=x_mat, y1=y_mat, th1=theta_mat))
  #print("Robot state",robot_theta)
  
  
##########################################################

rospy.init_node('Fol4')  # Defines a node with name of Follower
