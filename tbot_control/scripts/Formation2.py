...
def controller(min_value_1,min_index_1,min_value_2,min_index_2): 
  ...
  x_dot = K_for * (2*d*np.cos(30*np.pi/180)+min_value_1*np.cos(min_index_1*np.pi/180)+ min_value_2*np.cos(min_index_2*np.pi/180)) + K_tr*distance_to_goal_x
  y_dot = K_for * (min_value_1*np.sin(min_index_1*np.pi/180)+ min_value_2*np.sin(min_index_2*np.pi/180)) + K_tr*distance_to_goal_y
  ...
  print("Formation Errorx", 2*d*np.cos(30*np.pi/180)+min_value_1*np.cos(min_index_1*np.pi/180)+ min_value_2*np.cos(min_index_2*np.pi/180))
  print("Formation Errory", min_value_1*np.sin(min_index_1*np.pi/180)+ min_value_2*np.sin(min_index_2*np.pi/180))
...
def lidar_callback(msg):
  ...
  if (t_l > tlidar_data[0]+8):
    ...
    scipy.io.savemat('lid_robot2_FORM.mat', dict(t1lid=tlidar_mat, lid=lidar_mat))
...
def odom_callback(data):
  ...
  if (t_o > todom_data[0]+8):
    ...
    scipy.io.savemat('odom_robot2_FORM.mat', dict(t1=todom_mat, x1=x_mat, y1=y_mat, th1=theta_mat))
