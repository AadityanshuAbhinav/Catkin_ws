...
##########################################################
def LidarKal(P_prev1,P_prev2,M_prev1,M_prev2,V_prev,Z1,Z2,theta,dt):
    k = 0.05
    d_11 = M_prev1[0,0]
    phi_11 = M_prev1[1,0]
    ...
    P2 = np.dot(H-np.dot(K1,H),P_hat2)
...
def LidarMed(Z1,Z2,Z3):
    d = np.sort(np.array([Z1[0,0],Z2[0,0],Z3[0,0])]))
    ...
    M = np.array([[d[1],[phi[1]])  
    return M
  
#############################Edit only this function######

def lidar_callback(msg):
  global tlidar_data, lidar_data, d, P_prev1, P_prev2, M_prev1, M_prev2, ite, Z11, Z12, Z21, Z22
  ...
  if (t_l > tlidar_data[0]+8):
    ...
    scipy.io.savemat('lid_robot2_FORM3.mat', dict(t11lid=tlidar_mat, lid=lidar_mat))
  ...
  print(" Measurement1 " , Z12," Measurement2 " , Z22)
...
def lidar_callback1(msg):
  global tlidar_data, lidar_data, d, Z1_list, Z2_list
  #Make sense of the callback data
  #rostopic echo msg
  #0 to 359 aray index values with each value at a degree increment 
  #range should be in meters
