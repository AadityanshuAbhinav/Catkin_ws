...
class PathPlanner():
    def __init__(self):
        ...
        for base_frame in base_frames:
            try:
                self.tf_listener.waitForTransform(self.odom_frame, base_frame, rospy.Time(), rospy.Duration(1.0))
                self.base_frame = base_frame
                break
            except (tf.Exception, tf.ConnectivityException, tf.LookupException):
                pass
    
        # If none of the potential base frames were found, log an error and shut down ROS
        if not hasattr(self, 'base_frame'):
            rospy.loginfo("Cannot find transform between odom and base_link or base_footprint")
            rospy.signal_shutdown("tf Exception")
        else:
            rospy.loginfo("Initialized with base frame: %s", self.base_frame)
